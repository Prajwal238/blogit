import 'package:blogit/signup_screen.dart';
        ...
        BlocListener<FormBloc, FormsValidate>(
          listener: (context, state) {
            if (state.errorMessage.isNotEmpty) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text(Constants.textResetEmailSent)));
            } else if (state.isFormValid && !state.isLoading) {
              context.read<AuthBloc>().add(AuthStarted());
            } else if (state.isFormValidateFailed) {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text(Constants.textFixIssues)));
            }
          },
        ),
        ...
        return SizedBox(
          ...
          child: TextFormField(
              ...
              decoration: InputDecoration(
                ...
                errorText: !state.isEmailValid ? 'Please ensure the email entered is valid' : null,
                hintText: 'Email',
...
class _SubmitButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<FormBloc, FormsValidate>(
      builder: (context, state) {
        return (state.isLoading && !state.forgotPassword)
...
class _ForgotPassword extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return RichText(
        ...
        text: TextSpan(children: <TextSpan>[
          const TextSpan(
              text: Constants.textForgot,
              style: TextStyle(
                color: Constants.kDarkGreyColor,
              )),
          TextSpan(
              recognizer: TapGestureRecognizer()
                ..onTap = () => {context.read<FormBloc>().add(const ResetPassword(true))},
