...
class FormBloc extends Bloc<FormEvent, FormsValidate> {
  ...
  FormBloc(this._authRepo, this._dbRepo)
  ...
  _onFormSubmitted(FormSubmitted event, Emitter<FormsValidate> emit) async {
    UserModel user =
        UserModel(email: state.email, password: state.password, age: state.age, displayName: state.displayName);
  ...
  _updateUIAndSignUp(FormSubmitted event, Emitter<FormsValidate> emit, UserModel user) async {
    ...
    if (state.isFormValid) {
      try {
        ...
        UserModel updatedUser = user.copyWith(uid: authUser!.user!.uid, isVerified: authUser.user!.emailVerified);
        await _dbRepo.saveUserData(updatedUser);
      ...
      emit(state.copyWith(isLoading: false, isFormValid: false, isFormValidateFailed: true));
    }
  }

  _authenticateUser(FormSubmitted event, Emitter<FormsValidate> emit, UserModel user) async {
    emit(state.copyWith(
        errorMessage: "",
        isLoading: true,
        isFormValid: _isEmailValid(state.email) && _isPasswordValid(state.password)));
    if (state.isFormValid) {
      try {
        UserCredential? authUser = await _authRepo.signIn(user);
